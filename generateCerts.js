...
const csvData = `Name,Email,Certificate_Path
...
lines.forEach((line, index) => {
  const [name, email, path] = line.split(',');
  if (name && email && path) {
    certificates.push({
      id: (index + 1).toString(),
      recipientName: name.trim(),
      recipientEmail: email.trim(),
      certificatePath: path.trim(),
      ...
      certificateId: `DNA2025-${(index + 1).toString().padStart(3, '0')}`
    });
  }
});

// Generate TypeScript file content
const tsContent = `export interface Certificate {
  id: string;
...
// Write to file
fs.writeFileSync('./src/data/completeCertificates.ts', tsContent);
console.log(`Generated ${certificates.length} certificates in completeCertificates.ts`);
